...
static inline unsigned int ImCountSetBits(unsigned int v)
{
...
enum ImGuiInputFlagsPrivate_
{
    ...
    ImGuiInputFlags_RepeatRateNavTweak =
        1 << 3, // Repeat rate: Faster
                ...
                // - In theory ImGuiInputFlags_RepeatUntilOtherKeyPress may be a desirable default, but it would break
                // too many behavior so everything is opt-in.
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    ImGuiDockNodeFlags_NoResizeFlagsMask_ =
        (int) ImGuiDockNodeFlags_NoResize | ImGuiDockNodeFlags_NoResizeX | ImGuiDockNodeFlags_NoResizeY,
    ...
    ImGuiDockNodeFlags_LocalFlagsTransferMask_ =
        (int) ImGuiDockNodeFlags_NoDockingSplit | ImGuiDockNodeFlags_NoResizeFlagsMask_ |
        (int) ImGuiDockNodeFlags_AutoHideTabBar | ImGuiDockNodeFlags_CentralNode | ImGuiDockNodeFlags_NoTabBar |
...
struct ImGuiContext
{
    ...
    float FontSize;         // Currently bound font size == line height (== FontSizeBeforeScaling * io.FontGlobalScale *
                            // font->Scale * g.CurrentWindow->FontWindowScale).
...
struct IMGUI_API ImGuiWindow
{
  ...
  public:
    ...
    // float     CalcFontSize() const    { ImGuiContext& g = *Ctx; return g.FontSizeBeforeScaling * FontWindowScale *
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_Unsorted =
        1
        << 23, // [Docking] Trailing tabs with the _Unsorted flag will be sorted based on the DockOrder of their Window.
};
...
IMGUI_API void TranslateWindowsInViewport(ImGuiViewportP* viewport, const ImVec2& old_pos, const ImVec2& new_pos,
                                          const ImVec2& old_size, const ImVec2& new_size);
...
IMGUI_API void
ActivateItemByID(ImGuiID id); // Activate an item by ID (button, checkbox, tree node etc.). Activation is queued and
...
SetItemKeyOwner(ImGuiKey key,
                ImGuiInputFlags flags); // Set key owner to last item if it is hovered or active. Equivalent to 'if
...
IMGUI_API void
DockBuilderRemoveNodeChildNodes(ImGuiID node_id); // Remove all split/hierarchy. All remaining docked windows will be
...
IMGUI_API void RenderColorRectWithAlphaCheckerboard(ImDrawList* draw_list, ImVec2 p_min, ImVec2 p_max, ImU32 fill_col,
                                                    ...
                                                    ImDrawFlags flags = 0);
IMGUI_API void
RenderNavCursor(const ImRect& bb, ImGuiID id,
...
IMGUI_API bool
TreeNodeUpdateNextOpen(ImGuiID storage_id,
                       ImGuiTreeNodeFlags flags); // Return open state. Consume previous SetNextItemOpen() data, if any.
                                                  // May return true when logging.
...
IMGUI_API void
DebugLocateItemOnHover(ImGuiID target_id); // Only call on reaction to a mouse Hover: because only 1 at the same time!
...
static inline bool operator==(const ImTextureRef& lhs, const ImTextureRef& rhs)
{
...
{
    IM_ASSERT(index_idx < ImFontAtlasRectId_IndexMask_ &&
