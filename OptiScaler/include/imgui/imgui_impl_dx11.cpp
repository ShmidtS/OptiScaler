...
//  2025-05-07: DirectX11: Honor draw_data->FramebufferScale to allow for custom backends and experiment using it
//  (consistently with other renderer backends, even though in normal condition it is not set under Windows).
//  2025-02-24: [Docking] Added undocumented ImGui_ImplDX11_SetSwapChainDescs() to configure swap chain creation for
//  secondary viewports. 2025-01-06: DirectX11: Expose VertexConstantBuffer in ImGui_ImplDX11_RenderState. Reset
//  projection matrix in ImDrawCallback_ResetRenderState handler. 2024-10-07: DirectX11: Changed default texture sampler
//  to Clamp instead of Repeat/Wrap. 2024-10-07: DirectX11: Expose selected render state in ImGui_ImplDX11_RenderState,
//  which you can access in 'void* platform_io.Renderer_RenderState' during draw callbacks. 2022-10-11: Using 'nullptr'
//  instead of 'NULL' as per our switch to C++11. 2021-06-29: Reorganized backend to pull data from a single structure
//  to facilitate usage with multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX11: Replaced
//  direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18:
//  DirectX11: Change blending equation to preserve alpha in output buffer. 2019-08-01: DirectX11: Fixed code querying
//  the Geometry Shader state (would generally error with Debug layer enabled). 2019-07-21: DirectX11: Backup, clear and
//  restore Geometry Shader is any is bound when calling ImGui_ImplDX11_RenderDrawData. Clearing Hull/Domain/Compute
//  shaders without backup/restore. 2019-05-29: DirectX11: Added support for large mesh (64K+ vertices), enable
...
void ImGui_ImplDX11_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        __analysis_assume(backend_tex->pTexture != nullptr);
