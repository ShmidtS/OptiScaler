                                 ...
                                 bool is_for_current_view)
{
    ...
    else if (swapchain_panel)
    {
        ...
        auto winUI = LoadLibraryW(L"Windows.UI.dll");
        PFN_CreateControlInput SystemCreateControlInput = nullptr;
        if (winUI != nullptr)
            SystemCreateControlInput = (PFN_CreateControlInput) GetProcAddress(winUI, "CreateControlInput");

        HRESULT result = E_FAIL;
        if (SystemCreateControlInput != nullptr)
            result = SystemCreateControlInput(ABI::Windows::UI::Core::IID_ICoreInputSourceBase, &controlInput);
        if (SUCCEEDED(result))
        {
...
void ImGui_ImplUwp_NewFrame(ImVec2 displaySize)
{
    ...
    if (bd->CoreWindow)
    {
        ...
        // io.DisplaySize = ImVec2(rect.Width * io.DisplayFramebufferScale.x, rect.Height *
    ...
    else if (bd->SwapChainPanelElement)
    {
        ...
        io.DisplaySize = ImVec2(static_cast<float>(w * io.DisplayFramebufferScale.x),
                                static_cast<float>(h * io.DisplayFramebufferScale.y));
    }
