...
//  2025-04-30: Inputs: Fixed an issue where externally losing mouse capture (due to e.g. focus loss) would fail to
//  claim it again the next subsequent click. (#8594) 2025-03-26: [Docking] Viewports: fixed an issue when closing a
//  window from the OS close button (with io.ConfigViewportsNoDecoration = false) while user code was discarding the
//  'bool* p_open = false' output from Begin(). Because we allowed the Win32 window to close early, Windows destroyed it
//  and our imgui window became not visible even though user code was still submitting it. 2025-03-10: When dealing with
//  OEM keys, use scancodes instead of translated keycodes to choose ImGuiKey values. (#7136, #7201, #7206, #7306,
//  #7670, #7672, #8468) 2025-02-21: [Docking] WM_SETTINGCHANGE's SPI_SETWORKAREA message also triggers a refresh of
//  monitor list. (#8415) 2025-02-18: Added ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress mouse cursor support.
//  2024-11-21: [Docking] Fixed a crash when multiple processes are running with multi-viewports, caused by misusage of
//  GetProp(). (#8162, #8069) 2024-10-28: [Docking] Rely on property stored inside HWND to retrieve context/viewport,
//  should facilitate attempt to use this for parallel contexts. (#8069) 2024-09-16: [Docking] Inputs: fixed an issue
//  where a viewport destroyed while clicking would hog mouse tracking and temporary lead to incorrect update of
//  HoveredWindow. (#7971) 2024-07-08: Inputs: Fixed ImGuiMod_Super being mapped to VK_APPS instead of VK_LWIN||VK_RWIN.
//  (#7768) 2023-10-05: Inputs: Added support for extra ImGuiKey values: F13 to F24 function keys, app back/forward
//  keys. 2023-09-25: Inputs: Synthesize key-down event on key-up for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows
//  doesn't emit it (same behavior as GLFW/SDL). 2023-09-07: Inputs: Added support for keyboard codepage conversion for
//  when application is compiled in MBCS mode and using a non-Unicode window. 2023-04-19: Added
//  ImGui_ImplWin32_InitForOpenGL() to facilitate combining raw Win32/Winapi with OpenGL. (#3218) 2023-04-04: Inputs:
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport)
{
    ...
    vd->Hwnd = ::CreateWindowExW(vd->DwExStyle, L"ImGui FakePlatform", L"Untitled",
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (ctx == NULL)
        return DefWindowProc(hWnd, msg, wParam, lParam); // unlike ImGui_ImplWin32_WndProcHandler() we are called
