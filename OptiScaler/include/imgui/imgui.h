...
// - In v1.92.0 (2025/XX/XX): added ImTextureRef which carry either a ImTextureID either a pointer to internal texture
...
IMGUI_API void
ShowDemoWindow(bool* p_open = NULL); // create Demo window. demonstrate most ImGui features. call this to learn about
...
IMGUI_API bool
IsWindowHovered(ImGuiHoveredFlags flags =
...
IMGUI_API void
SetNextWindowScroll(const ImVec2& scroll); // set next window scrolling value (use < 0.0f to not affect a given axis).
...
// - To use old behavior (single size font): use 'PushFont(font, font->DefaultSize)' in call site, or set
// 'ImFontConfig::Flags |= ImFontFlags_UseDefaultSize' before calling AddFont().
IMGUI_API void PushFont(ImFont* font,
                        float font_size = -1); // use NULL as a shortcut to push default font. Use <0.0f to keep current
                                               // font size. Use font->DefaultSize to revert to font default size.
...
IMGUI_API void
PushTextWrapPos(float wrap_local_pos_x =
                    0.0f); // push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end
...
GetFontBaked(); // get current font bound at current size // == GetFont()->GetFontBaked(GetFontSize())
...
IMGUI_API void
SetCursorScreenPos(const ImVec2& pos);    // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND.
...
IMGUI_API void
TextLinkOpenURL(const char* label,
...
IMGUI_API void
SetNextItemStorageID(ImGuiID storage_id); // set id to use for open/close storage (default to same as item id).
...
Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0,
           const ImVec2& size =
               ImVec2(0, 0)); // "bool selected" carry the selection state (read-only). Selectable() is clicked is
...
TableGetSortSpecs(); // get latest sort specs for the table (NULL if not sorting).  Lifetime: don't hold on this pointer
                     // over multiple frames or past any subsequent call to BeginTable().
...
IMGUI_API int
TableGetHoveredColumn(); // return hovered column. return -1 when table is not hovered. return columns_count if the
                         // unused space at the right of visible columns is hovered. Can also use (TableGetColumnFlags()
...
GetColumnOffset(int column_index =
                    -1); // get position of column line (in pixels, from the left side of the contents region). pass -1
...
IMGUI_API void
SetTabItemClosed(const char* tab_or_docked_window_label); // notify TabBar or Docking system of a closed tab/window
...
IMGUI_API ImGuiID DockSpace(ImGuiID dockspace_id, const ImVec2& size = ImVec2(0, 0), ImGuiDockNodeFlags flags = 0,
                            const ImGuiWindowClass* window_class = NULL);
IMGUI_API ImGuiID DockSpaceOverViewport(ImGuiID dockspace_id = 0, const ImGuiViewport* viewport = NULL,
...
SetNextWindowClass(const ImGuiWindowClass* window_class); // set next window class (control docking compatibility +
                                                          // provide hints to platform backend via custom viewport flags
                                                          // and platform parent/child relationship)
...
IMGUI_API void
SetKeyboardFocusHere(int offset = 0); // focus keyboard on the next widget. Use positive 'offset' to access sub
...
IMGUI_API ImDrawList* GetBackgroundDrawList(
    ImGuiViewport* viewport = NULL); // get background draw list for the given viewport or viewport associated to the
                                     // current window. this draw list will be the first rendering one. Useful to
                                     // quickly draw shapes/text behind dear imgui contents.
IMGUI_API ImDrawList* GetForegroundDrawList(
    ImGuiViewport* viewport = NULL); // get foreground draw list for the given viewport or viewport associated to the
                                     // current window. this draw list will be the top-most rendered one. Useful to
...
IMGUI_API const char*
GetKeyName(ImGuiKey key); // [DEBUG] returns English name of the key. Those names are provided for debugging purpose and
                          // are not meant to be saved persistently nor compared.
...
IMGUI_API void
SetItemKeyOwner(ImGuiKey key); // Set key owner to last item ID if it is hovered or active. Equivalent to 'if
...
IMGUI_API bool
IsMouseDoubleClicked(ImGuiMouseButton button); // did mouse button double-clicked? Same as GetMouseClickedCount() == 2.
...
FindViewportByPlatformHandle(void* platform_handle); // this is a helper for backends. the type platform_handle is
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_DockHierarchy =
        1 << 4, // IsWindowHovered() only: Consider docking hierarchy (treat dockspace host as parent of docked window)
...
enum ImGuiDockNodeFlags_
{
    ...
    ImGuiDockNodeFlags_PassthruCentralNode =
        1
        << 3, //       // Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering
              //       everything excepted the Central Node when empty. Meaning the host window should probably use
              //       SetNextWindowBgAlpha(0.0f) prior to Begin() when using this. 2) When Central Node is empty: let
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasTextures =
        1 << 4, // Backend Renderer supports ImTextureData requests to create/update/destroy textures. This enables
                // incremental texture updates and texture reloads.
    ...
    ImGuiBackendFlags_HasMouseHoveredViewport =
        1 << 11, // Backend Platform supports calling io.AddMouseViewportEvent() with the viewport under the mouse. IF
                 // POSSIBLE, ignore viewports with the ImGuiViewportFlags_NoInputs flag (Win32 backend, GLFW 3.30+
                 // backend can do this, SDL backend cannot). If this cannot be done, Dear ImGui needs to use a flawed
...
struct ImGuiIO
{
    ...
    // Input Functions
    IMGUI_API void
    AddKeyEvent(ImGuiKey key, bool down); // Queue a new key down/up event. Key should be "translated" (as in, generally
    ...
    ImGuiKeyData
        KeysData[ImGuiKey_NamedKey_COUNT]; // Key state for all known keys. Use IsKeyXXX() functions to access this.
...
struct ImGuiSelectionExternalStorage
{
    ...
    IMGUI_API void
    ApplyRequests(ImGuiMultiSelectIO* ms_io); // Apply selection requests by using AdapterSetItemSelected() calls
};
...
struct ImDrawData
{
    ...
    IMGUI_API void
    AddDrawList(ImDrawList* draw_list); // Helper to add an external draw list into an existing ImDrawData.
...
{
    //------------------------------------------ core / backend ---------------------------------------
    int UniqueID; // w    -   // Sequential index to facilitate identifying a texture when debugging/printing. Unique
                  // per atlas.
...
struct ImFontConfig
{
    ...
    int OversampleH; // 0 (2)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto == 1 or 2 depending
                     // on size. Note the difference between 2 and 3 is minimal. You can reduce this to 1 for large
                     // glyphs save memory. Read https://github.com/nothings/stb/blob/master/tests/oversample/README.md
                     // for details.
    int OversampleV; // 0 (1)    // Rasterize at higher quality for sub-pixel positioning. 0 == auto == 1. This is not
    ...
    const ImWchar* GlyphExcludeRanges; // NULL     // Pointer to a VERY SHORT user-provided list of Unicode range (2
                                       ...
                                       // merging fonts with overlapping glyphs.
...
struct ImFontAtlas
{
    ...
    const ImFontLoader* FontLoader; // Font loader opaque interface (default to stb_truetype, can be changed to use
                                    // FreeType by defining IMGUI_ENABLE_FREETYPE). Don't set directly!
    ...
    unsigned int FontBuilderFlags;  // [FIXME: Should be called FontLoaderFlags] Shared flags (for all fonts) for font
                                    // loader. THIS IS BUILD IMPLEMENTATION DEPENDENT (e.g. . Per-font override is also
...
{
    ImFontFlags_None = 0,
    ImFontFlags_UseDefaultSize = 1 << 0, // Legacy compatibility: make PushFont() calls without explicit size use
                                         // font->DefaultSize instead of current font size.
...
struct ImFont
{
    ...
    float DefaultSize; // 4     // in  // Default font size passed to AddFont(). It's unlikely you should use this (use
                       // ImGui::GetFontBaked() to get font baked at current bound size).
    ImVector<ImFontConfig*> Sources; // 16    // in  // List of sources. Pointers within ContainerAtlas->Sources[]
    ImWchar EllipsisChar;            // 2-4   // out // Character used for ellipsis rendering ('...').
    ImWchar FallbackChar;            // 2-4   // out // Character used if a glyph isn't found (U+FFFD, '?')
    float Scale; // 4     // in  // Base font scale (~1.0f), multiplied by the per-window font scale which you can
    ...
    IMGUI_API ImVec2 CalcTextSizeA(float size, float max_width, float wrap_width, const char* text_begin,
                                   const char* text_end = NULL, const char** remaining = NULL); // utf8
    ...
    {
        return CalcWordWrapPosition(DefaultSize * scale, text, text_end, wrap_width);
    }
#endif
    ...
    IMGUI_API void ClearOutputData();
    IMGUI_API void
    AddRemapChar(ImWchar from_codepoint,
...
enum ImGuiViewportFlags_
{
    ...
    ImGuiViewportFlags_CanHostOtherWindows =
        1 << 11, // Viewport can host multiple imgui windows (secondary viewports are associated to a single window). //
                 // FIXME: In practice there's still probably code making the assumption that this is always and only on
                 // the MainViewport. Will fix once we add support for "no main viewport".
...
struct ImGuiPlatformIO
{
    ...
    ImVec2 (*Platform_GetWindowFramebufferScale)(
        ImGuiViewport* vp); // N . . . .  // Return viewport density. Always 1,1 on Windows, often 2,2 on Retina display
                            // on macOS/iOS. MUST BE INTEGER VALUES.
    ...
    ImVec4 (*Platform_GetWindowWorkAreaInsets)(
        ImGuiViewport* vp); // N . . . .  // (Optional) [BETA] Get initial work area inset for the viewport (won't be
                            // covered by main menu bar, dockspace over viewport etc.). Default to (0,0),(0,0).
...
IMGUI_API void Image(ImTextureRef tex_ref, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1,
                     ...
                     const ImVec4& border_col); // <-- border_col was removed in favor of ImGuiCol_ImageBorder.
...
// window_flags); } // Unnecessary as true == ImGuiChildFlags_Borders static inline bool BeginChild(ImGuiID id, const
