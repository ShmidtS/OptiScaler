...
//      FIXME: The transition from removing a viewport and moving the window in an existing hosted viewport tends to
//      flicker.
...
//  2025-05-07: DirectX12: Honor draw_data->FramebufferScale to allow for custom backends and experiment using it
//  (consistently with other renderer backends, even though in normal condition it is not set under Windows).
//  2025-02-24: DirectX12: Fixed an issue where ImGui_ImplDX12_Init() signature change from 2024-11-15 combined with
//  change from 2025-01-15 made legacy ImGui_ImplDX12_Init() crash. (#8429) 2025-01-15: DirectX12: Texture upload use
//  the command queue provided in ImGui_ImplDX12_InitInfo instead of creating its own. 2024-12-09: DirectX12: Let user
//  specifies the DepthStencilView format by setting ImGui_ImplDX12_InitInfo::DSVFormat. 2024-11-15: DirectX12:
//  *BREAKING CHANGE* Changed ImGui_ImplDX12_Init() signature to take a ImGui_ImplDX12_InitInfo struct. Legacy
//  ImGui_ImplDX12_Init() signature is still supported (will obsolete). 2024-11-15: DirectX12: *BREAKING CHANGE* User is
//  now required to pass function pointers to allocate/free SRV Descriptors. We provide convenience legacy fields to
//  pass a single descriptor, matching the old API, but upcoming features will want multiple. 2024-10-23: DirectX12:
//  Unmap() call specify written range. The range is informational and may be used by debug tools. 2024-10-07:
//  DirectX12: Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: DirectX12: Expose selected
//  render state in ImGui_ImplDX12_RenderState, which you can access in 'void* platform_io.Renderer_RenderState' during
//  draw callbacks. 2024-10-07: DirectX12: Compiling with '#define ImTextureID=ImU64' is unnecessary now that dear imgui
//  defaults ImTextureID to u64 instead of void*. 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to
//  C++11. 2021-06-29: Reorganized backend to pull data from a single structure to facilitate usage with
//  multiple-contexts (all g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX12: Replaced direct access to
//  ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX12: Change
//  blending equation to preserve alpha in output buffer. 2021-01-11: DirectX12: Improve Windows 7 compatibility (for
//  D3D12On7) by loading d3d12.dll dynamically. 2020-09-16: DirectX12: Avoid rendering calls with zero-sized scissor
//  rectangle since it generates a validation layer warning. 2020-09-08: DirectX12: Clarified support for building on
//  32-bit systems by redefining ImTextureID. 2019-10-18: DirectX12: *BREAKING CHANGE* Added extra ID3D12DescriptorHeap
//  parameter to ImGui_ImplDX12_Init() function. 2019-05-29: DirectX12: Added support for large mesh (64K+ vertices),
//  enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX12: Added support for special
...
// data. Main viewport created by application will only use the Resources field. Secondary viewports created by this
// backend will use all the fields (including Window fields),
...
// Functions
static void ImGui_ImplDX12_SetupRenderState(ImDrawData* draw_data, ID3D12GraphicsCommandList* command_list,
...
void ImGui_ImplDX12_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        bd->InitInfo.SrvDescriptorAllocFn(&bd->InitInfo, &backend_tex->hFontSrvCpuDescHandle,
        ...
        bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COPY_DEST,
        ...
        need_barrier_before_copy = false; // Because this is a newly-created texture it will be in
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        UINT upload_pitch_dst =
            (upload_pitch_src + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u) & ~(D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u);
        ...
        __analysis_assume(event != nullptr);

        // FIXME-OPT: Create once and reuse?
        ID3D12CommandAllocator* cmdAlloc = nullptr;
        hr = bd->pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&cmdAlloc));
        IM_ASSERT(SUCCEEDED(hr));
        __analysis_assume(cmdAlloc != nullptr);
        ...
        for (int y = 0; y < upload_h; y++)
            memcpy((void*) ((uintptr_t) mapped + y * upload_pitch_dst), tex->GetPixelsAt(upload_x, upload_y + y),
                   upload_pitch_src);
        ...
        // - To remove this may need to create NumFramesInFlight x ImGui_ImplDX12_FrameContext in backend data (mimick
        // docking version)
    ...
    // Create the root signature
    {
        ...
        if (d3d12_dll == nullptr)
        {
            ...
            const char* localD3d12Paths[] = {
                ".\\d3d12.dll", ".\\d3d12on7\\d3d12.dll", ".\\12on7\\d3d12.dll"
        ...
        PFN_D3D12_SERIALIZE_ROOT_SIGNATURE D3D12SerializeRootSignatureFn =
            (PFN_D3D12_SERIALIZE_ROOT_SIGNATURE) (void*) ::GetProcAddress(d3d12_dll, "D3D12SerializeRootSignature");
                         ...
                         D3D12_GPU_DESCRIPTOR_HANDLE font_srv_gpu_desc_handle)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    io.BackendFlags &= ~ImGuiBackendFlags_RendererHasTextures; // Using legacy ImGui_ImplDX12_Init() call with 1 SRV
...
void ImGui_ImplDX12_Shutdown(bool shutdown_platform, bool invalidate)
{
    ...
    {
        // We could just call ImGui_ImplDX12_DestroyWindow(main_viewport) as a convenience but that would be misleading
        // since we only use data->Resources[]
...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    IM_ASSERT(res == S_OK);
    __analysis_assume(vd->CommandQueue != nullptr);

    // Create command allocator.
    for (UINT i = 0; i < bd->numFramesInFlight; ++i)
    {
        res = bd->pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                                     IID_PPV_ARGS(&vd->FrameCtx[i].CommandAllocator));
        IM_ASSERT(res == S_OK);
    }

    // Create command list.
    res = bd->pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, vd->FrameCtx[0].CommandAllocator,
...
static void ImGui_ImplDX12_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    if (!(viewport->Flags & ImGuiViewportFlags_NoRendererClear))
        cmd_list->ClearRenderTargetView(vd->FrameCtx[back_buffer_idx].RenderTargetCpuDescriptors, (float*) &clear_color,
                                        0, nullptr);
