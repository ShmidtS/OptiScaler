...
#pragma GCC diagnostic ignored                                                                                         \
    "-Wsubobject-linkage" // warning: 'xxxx' has a field 'xxxx' whose type uses the anonymous namespace
#endif
#endif
...
bool ImGui_ImplFreeType_FontSrcData::InitFont(FT_Library ft_library, ImFontConfig* src,
                                              ImGuiFreeTypeBuilderFlags extra_font_builder_flags)
{
    FT_Error error = FT_New_Memory_Face(ft_library, (uint8_t*) src->FontData, (uint32_t) src->FontDataSize,
                                        (uint32_t) src->FontNo, &FtFace);
    if (error != 0)
        return false;
...
bool ImGui_ImplFreeType_FontBakedInit(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
                                      void* loader_data_for_baked_src)
{
    IM_UNUSED(atlas);
    const float size =
        baked->Size *
        (src->SizePixels / baked->ContainerFont->Sources[0]->SizePixels); // FIXME-NEWATLAS: Should tidy up that a bit
    ...
    req.type = (bd_font_data->UserFlags & ImGuiFreeTypeBuilderFlags_Bitmap) ? FT_SIZE_REQUEST_TYPE_NOMINAL
...
void ImGui_ImplFreeType_FontBakedDestroy(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
                                         void* loader_data_for_baked_src)
{
    IM_UNUSED(atlas);
    IM_UNUSED(baked);
...
bool ImGui_ImplFreeType_FontBakedLoadGlyph(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked,
                                           void* loader_data_for_baked_src, ImWchar codepoint, ImFontGlyph* out_glyph)
{
    ...
    FT_Render_Mode render_mode =
        (bd_font_data->UserFlags & ImGuiFreeTypeBuilderFlags_Monochrome) ? FT_RENDER_MODE_MONO : FT_RENDER_MODE_NORMAL;
