...
{
    ImGuiFreeTypeBuilderFlags_NoHinting = 1 << 0,  // Disable hinting. This generally generates 'blurrier' bitmap glyphs
                                                   // when the glyph are rendered in any of the anti-aliased modes.
    ...
    ImGuiFreeTypeBuilderFlags_LightHinting =
        1 << 3, // A lighter hinting algorithm for gray-level modes. Many generated glyphs are fuzzier but better
                // resemble their original shape. This is achieved by snapping glyphs to the pixel grid only vertically
                // (Y-axis), as is done by Microsoft's ClearType and Adobe's proprietary font renderer. This preserves
...
// However, as FreeType does lots of allocations we provide a way for the user to redirect it to a separate memory heap
// if desired.
...
// IMGUI_API const ImFontBuilderIO* GetBuilderForFreeType(); // Renamed/changed in 1.92. Change 'io.Fonts->FontBuilderIO
// = ImGuiFreeType::GetBuilderForFreeType()' to 'io.Fonts.FontLoader = ImGuiFreeType::GetFontLoader()' if you need
// runtime selection. static inline bool BuildFontAtlas(ImFontAtlas* atlas, unsigned int flags = 0) {
// atlas->FontBuilderIO = GetBuilderForFreeType(); atlas->FontBuilderFlags = flags; return atlas->Build(); } // Prefer
// using '#define IMGUI_ENABLE_FREETYPE'
#endif
} // namespace ImGuiFreeType
